...
def train_model(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (epoch + 1) in save_checkpoints:
            ...
            save_checkpoint(
                model, optimizer, epoch + 1, loss.item(), chkpnt_path
            )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train PoseNet with optional checkpointing"
    )
    parser.add_argument(
        "--images_txt_path", required=True, help="Path to COLMAP's images.txt"
    )
    parser.add_argument(
        "--frames_root",
        required=True,
        help="Path to directory with image files",
    )
    parser.add_argument(
        "--output_path", required=True, help="Path to save trained model"
    )
    parser.add_argument(
        "--epochs", type=int, default=20, help="Number of training epochs"
    )
    parser.add_argument(
        "--checkpoint_path", default=None, help="Path to save/load checkpoint"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
